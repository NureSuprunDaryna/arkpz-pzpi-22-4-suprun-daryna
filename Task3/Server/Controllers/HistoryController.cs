...
namespace Server.Controllers
{
    ...
    public class HistoryController : ControllerBase
    {
        ...
        [HttpPost("report/{userID}")]
        public async Task<IActionResult> GenerateReport(string userID, [FromBody] ReportRequestModel request)
        {
            var currentUserId = HttpContext.Session.GetString("UserId");
            if (string.IsNullOrEmpty(currentUserId))
            {
            ...
            if (request == null)
            {
                _logger.LogError("Invalid report request for user {UserNickname}.", userID);
                return BadRequest(new { message = "Report request data is required." });
            }

            _logger.LogInformation("Admin {AdminName} is generating report for user {UserNickname}.", currentUser.UserName, userID);

            var reportRequest = new ReportRequest();
            reportRequest.MapFrom(request);

            var result = await _historyService.GenerateReport(userID, reportRequest);

            if (!result.IsSuccessful || result.Data == null)
            {
                _logger.LogError("Failed to generate report for user {UserNickname}.", userID);
                return BadRequest(new { message = "Failed to generate report." });
            }

            _logger.LogInformation("Report generated successfully for user {UserNickname}.", userID);
            return Ok(new { message = "Report generated successfully.", report = result.Data });
        }

        // Генерація PDF звіту (тільки для адміністраторів)
        [HttpPost("report/pdf/{userID}")]
        public async Task<IActionResult> GenerateReportPdf(string userID, [FromBody] ReportRequestModel request)
        {
            ...
            _logger.LogInformation("Admin {AdminName} is generating PDF report for user {UserNickname}.", currentUser.UserName, userID);

            var reportRequest = new ReportRequest();
            reportRequest.MapFrom(request);

            var result = await _historyService.GenerateReportPdf(userID, reportRequest);

            if (!result.IsSuccessful || result.Data == null)
            {
                _logger.LogError("Failed to generate PDF report for user {UserNickname}.", userID);
                return NotFound(new { message = "Failed to generate PDF report." });
            }

            _logger.LogInformation("PDF report generated successfully for user {UserNickname}.", userID);
            return File(result.Data, "application/pdf", $"{userID}_Report.pdf");
        }
