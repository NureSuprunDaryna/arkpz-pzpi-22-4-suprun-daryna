    ...
    public class ElixirService : GenericService<Elixir>, IElixirService
    {
        ...
        public async Task<Result<Elixir>> CreateElixir(Elixir elixir)
        {
            ...
            try
            {
                ...
                var author = await _userManager.FindByIdAsync(elixir.AuthorId);
                if (author == null)
                {
                    _logger.LogError("Failed to create elixir - Author with ID {AuthorId} not found.", elixir.AuthorId);
                    return new Result<Elixir>(false);
                }

                if (elixir.Description == null)
                {
                    _logger.LogError("Failed to create elixir - Description not found.");
                    return new Result<Elixir>(false);
                }

                var existingElixir = await _elixirRepository.GetAsync(e => e.Name == elixir.Name);
                if (existingElixir.Any())
                {
                    _logger.LogError("Failed to create elixir - Elixir with name {Name} already exists.", elixir.Name);
                    return new Result<Elixir>(false);
                }
        ...
        public async Task<Result> EditElixir(Elixir elixir)
        {
        ...
        */
        ...
        public async Task<Result> UpdateElixirComposition(int elixirId, List<(int noteId, string noteCategory, decimal proportion)> updatedNotes)
        {
            ...
            try
            {
                ...
                var currentComposition = elixir.ElixirComposition.ToList();
                var currentNoteIds = currentComposition.Select(ec => ec.NoteId).ToList();
                var updatedNoteIds = updatedNotes.Select(n => n.noteId).ToList();

                var notesToAdd = updatedNotes.Where(n => !currentNoteIds.Contains(n.noteId)).ToList();

                var notesToRemove = currentComposition.Where(c => !updatedNoteIds.Contains(c.NoteId)).ToList();

                foreach (var existingNote in currentComposition)
                {
                    var updatedNote = updatedNotes.FirstOrDefault(n => n.noteId == existingNote.NoteId);
                    if (updatedNote != default)
                    {
                        existingNote.NoteCategory = updatedNote.noteCategory;
                        existingNote.Proportion = updatedNote.proportion;
                    }
                }

                foreach (var noteToRemove in notesToRemove)
                {
                    elixir.ElixirComposition.Remove(noteToRemove);
                }

                foreach (var noteToAdd in notesToAdd)
                {
                    elixir.ElixirComposition.Add(new ElixirComposition
                    {
                        ElixirId = elixirId,
                        NoteId = noteToAdd.noteId,
                        NoteCategory = noteToAdd.noteCategory,
                        Proportion = noteToAdd.proportion
                    });
                }
