...
namespace BLL.Services
{
    ...
    : base(unitOfWork, unitOfWork.HistoryRepository)
        ...
        public async Task<Result<List<History>>> GetUserHistory(string userId, DateTime? date, string? elixirName, List<string>? noteNames, List<string>? elixirNames)
        {
            _logger.LogInformation("Filtering history for user {Id}.", userId);
            ...
            try
            {
                ...
                var query = await _repository.GetAsync(h => h.Elixir.AuthorId == userId);
                ...
                if (!query.Any())
                {
                    _logger.LogWarning("No matching history records found for user {Id}.", userId);
                    return new Result<List<History>>(false);
                }

                _logger.LogInformation("Successfully filtered history for user {Id}.", userId);
                return new Result<List<History>>(true, query);
            }
            catch (Exception ex)
            {
                _logger.LogError("Error filtering history for user {Id}. Error: {Message}", userId, ex.Message);
                return new Result<List<History>>(false);
            }
        }
        ...
        public async Task<Result<byte[]>> GenerateReportPdf(string userNickname, ReportRequest request)
        {
            ...
            try
            {
                ...
                {
                    document.Add(new Paragraph(section.Key).SetFontSize(16));
                    if (section.Value is Dictionary<string, object> sectionDetails)
                    {
                        foreach (var detail in sectionDetails)
                        {
                            document.Add(new Paragraph($"{detail.Key}: {detail.Value}").SetFontSize(12));
                        }
                    }
                    ...
                    {
                        document.Add(new Paragraph(section.Value.ToString()).SetFontSize(12));
                    }
